...
const API_BASE_URL =
  process.env.NODE_ENV === "production" ? "https://mvpshell.vercel.app" : "";
...
function isPdfUrl(url: string): boolean {
  return (
    url.toLowerCase().includes("/uploads/evidence/") ||
...
export default function MarketPage({
  ...
  // For Buy Yes
  const { mutate: sendBuyYesTransaction, status: buyYesStatus } =
    useSendTransaction();
  // For Buy No
  const { mutate: sendBuyNoTransaction, status: buyNoStatus } =
    useSendTransaction();
  ...
  const userDeposit = userTokenBalance
    ? BigInt(userTokenBalance) * BigInt(2 ** 64)
    : 0n;

  const handleApproveIfNeeded = async () => {
    if (isAllowancePending || !account?.address) {
      console.error("Approval: allowance pending or no account", {
        isAllowancePending,
        account,
      });
      return false;
  ...
  const handleBuyYesWithApproval = async (amount: string) => {
    ...
    const cashAvailable = userTokenBalance
      ? Number(userTokenBalance) / 1e18
      : 0;
    ...
    if (approved || (allowance && Number(allowance) >= userDeposit)) {
      ...
      console.error("Approval failed or not completed", {
        approved,
        allowance,
        userDeposit,
      });
  ...
  const handleBuyYes = async (amount: string) => {
    ...
    const discountedBetAmountInUSDC = BigInt(
      Math.floor(discountedBetAmount * 1e18)
    );
    ...
    try {
      const sharesResult = await readContract({
        contract: marketContract,
        method:
          "function calculateSharesFromBetAmount(uint256 _outcome, uint256 _betAmount) view returns (uint256 shares)",
    ...
    const transaction = prepareContractCall({
      contract: marketContract,
      method:
        "function buy(uint256 _outcome, uint256 _betAmount) returns (uint256 shares)",
  ...
  const handleSellYes = (amount: string) => {
    ...
    const transaction = prepareContractCall({
      contract: marketContract,
      method:
        "function sell(uint256 _outcome, int128 _amount) returns (int128 _price)",
      ...
      onError: (error) => {
        ...
        if (error.message) {
          ...
          if (msg.includes("insufficient funds")) {
            ...
            errorMessage =
              "Contract execution failed. Insufficient shares or invalid amount.";
            ...
            errorMessage =
              "Contract error: Insufficient shares or invalid sell amount. Check your balance.";
          } else if (msg.includes("abi error signature not found")) {
            errorMessage =
              "Contract error: Invalid sell request. Check your shares balance and try a smaller amount.";
          }
        }
  ...
  const handleSellNo = (amount: string) => {
    ...
    const transaction = prepareContractCall({
      contract: marketContract,
      method:
        "function sell(uint256 _outcome, int128 _amount) returns (int128 refund)",
  ...
  const handleAutoGetPrice = useCallback(
    async (outcome: number, amount: number, isSellMode: boolean = false) => {
      ...
      try {
        if (isSellMode) {
          ...
          const result = await readContract({
            contract: marketContract,
            method:
              "function calculateSellRefund(uint256 _outcome, uint256 _amount) view returns (uint256 refund)",
  ...
  const fetchUserVotes = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/user-votes?walletAddress=${account.address}&marketId=${market.id}`
      );
      if (res.ok) {
        const userVoteData = await res.json();
        // Assuming the backend returns an array of evidence IDs the user has voted on
        const votedEvidenceIds: Set<number> = new Set(
          userVoteData.map((vote: { evidenceId: number }) =>
            Number(vote.evidenceId)
          )
        );
  ...
  const sortedYesEvidence = (Array.isArray(evidence) ? evidence : [])
    .filter((ev) => ev.type === "yes")
    .sort((a, b) => b.netVotes - a.netVotes);
  const sortedNoEvidence = (Array.isArray(evidence) ? evidence : [])
    .filter((ev) => ev.type === "no")
    .sort((a, b) => b.netVotes - a.netVotes);

  // Show limited evidence initially
  const yesToShow = showAllYes
    ? sortedYesEvidence
    : sortedYesEvidence.slice(0, 5);
  ...
  if (
    ...
    if (priceResult !== undefined && !isPricePending && !priceError) {
      if (mode === "sell") {
        ...
        console.log(
          "avgPrice calculation: (refundReceived / shareAmount) * 100 =",
          avgPriceInCents
        );
        ...
        console.log(
          "Discounted amount used in contract call:",
          amountNum * 0.98
        );
        console.log("Overround adjustment: 2% discount applied");
        console.log(
          "avgPrice calculation: (amountNum / sharesReceived) * 100 =",
          avgPriceInCents
        );
        console.log("avgPriceDisplay (rounded):", avgPriceDisplay);
    ...
    try {
      ...
      if (account?.address) {
        try {
          ...
          console.error(
            "Error fetching latest balances after transaction:",
            err
          );
        }
      }
  ...
  const yesVotingPower = account?.address
    ? yesShares - noShares > 0
      ? Math.max(1, yesShares - noShares)
      : 0
    : 0;
  const noVotingPower = account?.address
    ? noShares - yesShares > 0
      ? Math.max(1, noShares - yesShares)
      : 0
    : 0;

  // For conditional tokens approval (for selling)
  const { mutate: sendSetApprovalForAll } = useSendTransaction();
  const { data: isOperatorApproved, refetch: refetchOperatorApproval } =
    useReadContract({
      contract: conditionalTokensContract,
      method:
  ...
  const recordNewOdds = useCallback(async () => {
    try {
      ...
      if (!oddsResponse.ok) {
        console.error(
          "Failed to record odds to database:",
          oddsResponse.status,
          oddsResponse.statusText
        );
      } else {
        console.log("Recorded new odds to database after trade:", {
          yesProbability,
          noProbability,
        });
      }
          ...
          <div className="bg-white p-6 mx-4" style={{ maxWidth: "375px" }}>
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-4">
                Please connect a wallet to begin trading. Click connect button
                in the top right and sign-in using any account.
              </p>
              <button
        ...
        <div className="max-w-[1600px] mx-auto w-full">
          <div className="w-full flex flex-col lg:flex-row gap-0.5 pt-2.5">
            ...
            <div className="bg-white p-2.5 sm:pl-8 sm:pr-2.5 w-full lg:w-[calc(100%-350px)] mb-8 lg:mb-0 flex flex-col">
              ...
              <h2 className="text-2xl font-bold mb-6 text-[#171A22]">
                {market.title}
              </h2>
              <div className="mb-2">
                <span className="text-lg font-semibold text-[#171A22]">
                  Market Odds
                </span>
              </div>
                ...
                {typeof oddsYes === "bigint" && typeof oddsNo === "bigint" ? (
                  <>
                    <span className="text-green-600">
                      Yes:{" "}
                      {Math.round((Number(oddsYes) / Math.pow(2, 64)) * 100)}%
                    </span>
                    <span className="mx-2 text-red-600">
                      No: {Math.round((Number(oddsNo) / Math.pow(2, 64)) * 100)}
                      %
                    </span>
                  </>
                ) : (
              ...
              <div className="mt-8">
                ...
                {splitRules(showRules ? rulesFull : firstLine).map(
                  (para, i) => (
                    <p key={i} className="text-gray-600 text-base mb-2">
                      {para}
                    </p>
                  )
                )}
                ...
                <div className="bg-white p-0 w-full max-w-[600px]">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex gap-2">
                      <button
                        className={`py-1 px-3 text-base rounded-full border ${
                          mode === "buy"
                            ? "bg-gray-100 text-green-600 border-gray-300 font-bold"
                            : "bg-white text-black border-gray-300 font-normal"
                        }`}
                        onClick={() => {
                          setMode("buy");
                          setAmount("");
                        ...
                        <div className="text-sm font-semibold text-green-600 flex flex-col items-end">
                          <span className="text-green-600">
                            ...
                            {isBalanceLoading
                              ? "..."
                              : outcome1Balance !== "--" &&
                                outcome1Balance !== "Error"
                              ? parseFloat(outcome1Balance).toFixed(2)
                              : outcome1Balance}
                          </span>
                          <span className="text-red-600">
                            ...
                            {isBalanceLoading
                              ? "..."
                              : outcome2Balance !== "--" &&
                                outcome2Balance !== "Error"
                              ? parseFloat(outcome2Balance).toFixed(2)
                              : outcome2Balance}
                          </span>
                        </div>
                      )}
                  ...
                  <div className="text-lg font-bold mb-2">
                    {mode === "buy" ? (
                      <>
                        Bet Amount (<DenariusSymbol size={12} />)
                      </>
                    ) : (
                      "Sell Shares"
                    )}
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full ${
                        mode === "buy" && amount ? "pl-4" : "pl-3"
                      } pr-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black text-base mb-4`}
                    />
                    {mode === "buy" && amount && (
                      <div
                  ...
                  <div className="flex flex-row w-full mb-4 gap-2">
                    <button
                      type="button"
                      className={`font-semibold px-6 py-2 rounded-lg shadow transition disabled:opacity-50 bg-green-600 hover:bg-green-700 text-white w-1/2 ${
                        selectedOutcome === "yes" ? "ring-2 ring-black" : ""
                      }`}
                      onClick={() => {
                        ...
                        if (sellAllClicked) {
                          ...
                          console.log(
                            "Sell All active - updated input to Yes shares:",
                            yesShares
                          );
                        } else {
                          setSellAllClicked(false);
                    ...
                    >
                      {mode === "buy"
                        ? buyYesStatus === "pending"
                          ? "Buying..."
                          : `Yes ${formatOddsToCents(oddsYes)}`
                        : buyYesStatus === "pending"
                        ? "Selling..."
                        : `Yes ${formatOddsToCents(oddsYes)}`}
                    </button>
                    <button
                      type="button"
                      className={`font-semibold px-6 py-2 rounded-lg shadow transition disabled:opacity-50 bg-red-600 hover:bg-red-700 text-white w-1/2 ${
                        selectedOutcome === "no" ? "ring-2 ring-black" : ""
                      }`}
                      onClick={() => {
                        ...
                        if (sellAllClicked) {
                          ...
                          console.log(
                            "Sell All active - updated input to No shares:",
                            noShares
                          );
                        } else {
                          setSellAllClicked(false);
                  ...
                  {mode === "sell" &&
                    ((outcome1Balance !== "--" &&
                      outcome1Balance !== "Error" &&
                      parseInt(outcome1Balance) > 0) ||
                      (outcome2Balance !== "--" &&
                        outcome2Balance !== "Error" &&
                        parseInt(outcome2Balance) > 0)) && (
                      <div className="flex justify-start mb-4">
                        <button
                          className={`py-1 px-3 text-base rounded-full border transition-colors ${
                            sellAllClicked
                              ? "bg-gray-100 text-green-600 border-gray-300 font-bold"
                              : "bg-white text-black border-gray-300 font-normal hover:bg-gray-50"
                          }`}
                          onClick={() => {
                            if (sellAllClicked) {
                              ...
                              setSelectedOutcome(null);
                              console.log(
                                "Sell All button deactivated, input cleared, outcome deselected"
                              );
                            } else {
                              ...
                              if (selectedOutcome === "yes") {
                                ...
                                setAmount(truncateOneDigit(yesShares));
                                console.log(
                                  "User already selected Yes - inputting Yes shares:",
                                  yesShares
                                );
                                ...
                                setAmount(truncateOneDigit(noShares));
                                console.log(
                                  "User already selected No - inputting No shares:",
                                  noShares
                                );
                              } else {
                                // No outcome selected - default to larger position (truncated)
                                const largerPosition = Math.max(
                                  yesShares,
                                  noShares
                                );
                                const outcomeWithMoreShares =
                                  yesShares >= noShares ? "yes" : "no";

                                setAmount(truncateOneDigit(largerPosition));
                                setSelectedOutcome(outcomeWithMoreShares);

                                console.log(
                                  "No outcome selected - defaulting to larger position:",
                                  largerPosition
                                );
                                console.log(
                                  "Automatically selected outcome:",
                                  outcomeWithMoreShares
                                );
                              }
                              ...
                              console.log(
                                "Final amount set to input:",
                                yesShares > noShares ? yesShares : noShares
                              );
                            }
                          }}
                        >
                  ...
                  {amount && !isNaN(Number(amount)) && selectedOutcome && (
                    <>
                      ...
                      <div className="text-[1.15rem] font-medium text-black">
                        {mode === "buy" ? "Max. Win:" : "Receive:"}{" "}
                        <span className="text-green-600 font-bold">
                          <span className="font-normal">
                            <DenariusSymbol size={13} />
                          </span>{" "}
                          {payoutDisplay}
                        </span>
                      </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (selectedOutcome === "yes") {
                            if (mode === "buy") {
                              handleBuyYesWithApproval(amount);
                            } else {
                              handleSellYesWithApproval(amount);
                            }
                          } else if (selectedOutcome === "no") {
                            if (mode === "buy") {
                              handleBuyNoWithApproval(amount);
                            } else {
                              handleSellNoWithApproval(amount);
                            }
                          }
                        }}
                      >
                  ...
                  {buyFeedback && (
                    <div
                      className={`text-center mt-4 font-semibold flex items-center justify-center gap-1 ${
                        buyFeedback.includes("Purchase Successful") ||
                        buyFeedback.includes("Sale Successful") ||
                        buyFeedback.includes("🎉")
                          ? "text-green-600"
                          : buyFeedback.includes("Preparing transaction") ||
                            buyFeedback.includes("Checking approval") ||
                            buyFeedback.includes("Transaction submitted")
                          ? "text-black"
                          : "text-red-600"
                      }`}
                    >
                ...
                <Tab.Group>
                  <Tab.List className="flex w-full mb-6 bg-gray-50 rounded-lg">
                    <Tab
                      className={({ selected }: { selected: boolean }) =>
                        `flex-1 px-6 py-2 rounded-lg font-medium text-sm transition focus:outline-none ${
                          selected
                            ? "bg-white text-[#171A22] shadow"
                  ...
                  <Tab.Panels>
                                ...
                                <div className="flex flex-col items-center mr-4 select-none">
                                  <button
                                    className={`text-lg p-0 mb-1 transition-all duration-200 ${
                                      votingEvidenceId === evidence.id
                                        ? "opacity-50 cursor-not-allowed"
                                        : ""
                                    } ${
                                      userVotes.has(evidence.id)
                                        ? "bg-green-600 rounded-lg p-1"
                                        : ""
                                    }`}
                                    onClick={() =>
                                      handleVote(evidence.id, "yes")
                                    }
                                    aria-label={
                                      userVotes.has(evidence.id)
                                        ? "Remove vote"
                                  ...
                                  <div className="flex flex-col items-center">
                                    <div
                                      className={`bg-black text-white rounded-full px-1.5 py-0.5 text-xs font-semibold mb-1`}
                                      style={{
                                        minWidth: "1.48rem",
                                        textAlign: "center",
                                      }}
                                    ...
                                    <div
                                      className="text-green-600 text-xs font-semibold min-h-[1.25rem]"
                                      style={{ minHeight: "1.25rem" }}
                                    >
                                      {userVotes.has(evidence.id) ? (
                                        `+${getUserVotingContribution(
                                          evidence.id,
                                          "yes"
                                        )}`
                                      ) : (
                                  ...
                                  <div className="flex items-center mb-2">
                                    ...
                                    <div className="flex-1">
                                      {evidence.url ? (
                                        ...
                                        >
                                          {evidence.title} (
                                          {isPdfUrl(evidence.url)
                                            ? "pdf"
                                            : getDomain(evidence.url)}
                                          )
                                        </a>
                                      ) : (
                                        <span className="font-bold text-[#171A22] text-[95%]">
                                          {evidence.title}
                                        </span>
                                      )}
                                      <button
                                        ...
                                        onClick={() =>
                                          router.push(
                                            `/evidence/discussion/${evidence.id}`
                                          )
                                        }
                                      >
                                        View Discussion
                    ...
                    <Tab.Panel>
                      ...
                      >
                        <div>
                          <label className="block font-medium text-gray-700 mb-2 text-[95%]">
                            Evidence Type
                          </label>
                        ...
                        {evidenceSuccessMessage && (
                          <div className="text-black font-semibold text-center mb-2">
                            {evidenceSuccessMessage}
                          </div>
                        )}
                        <button
            ...
            <div className="hidden lg:block bg-white shadow p-4 sm:max-w-4xl sm:mx-auto sm:p-8 lg:w-[315px] lg:self-start">
              ...
              <div className="text-lg font-bold mb-2">
                {mode === "buy" ? (
                  <>
                    Bet Amount (<DenariusSymbol size={11} />)
                  </>
                ) : (
                  "Sell Shares"
                )}
              ...
              {mode === "sell" &&
                ((outcome1Balance !== "--" &&
                  outcome1Balance !== "Error" &&
                  parseFloat(outcome1Balance) > 0) ||
                  (outcome2Balance !== "--" &&
                    outcome2Balance !== "Error" &&
                    parseFloat(outcome2Balance) > 0)) && (
                  <div className="flex justify-start mb-4">
                    <button
                      className={`py-1 px-3 text-base rounded-full border transition-colors ${
                        sellAllClicked
                  ...
                  onClick={() => {
                    ...
                    setTimeout(() => {
                      const connectButton = document.querySelector(
                        '[data-testid="connect-button"], button[class*="bg-black"]'
                      ) as HTMLElement;
                      if (connectButton) {
